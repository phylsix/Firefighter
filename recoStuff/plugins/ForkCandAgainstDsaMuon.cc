...
ForkCandAgainstDsaMuon::ForkCandAgainstDsaMuon( const edm::ParameterSet& iC )
    : srcToken_( consumes<edm::View<reco::Candidate>>(
          iC.getParameter<edm::InputTag>( "src" ) ) ),
      matchedToken_( consumes<edm::View<reco::Candidate>>(
...
ForkCandAgainstDsaMuon::produce( edm::Event&            iEvent,
  ...
  for ( int isrc( 0 ); itsrc != edsrc; ++itsrc, ++isrc ) {
    ...
    const reco::PFCandidate* iPF =
        dynamic_cast<const reco::PFCandidate*>( &*itsrc );
    if ( iPF->particleId() != reco::PFCandidate::ParticleType::mu ) {
      continue;
    }

    matchIdx[ isrc ] =
        algo_.match( *itsrc, *matchedHdl_, deltaRs[ isrc ], deltaEtas[ isrc ],
                     deltaPhis[ isrc ], deltaLocalPos[ isrc ],
                     deltaPtRel[ isrc ], chi2[ isrc ] );
  }
  ...
  for ( int im( 0 ); im != (int)matchedHdl_->size(); ++im ) {
    const reco::PFCandidate* iPF =
        dynamic_cast<const reco::PFCandidate*>( &( *matchedHdl_ )[ im ] );
