...
HLTFilter::HLTFilter(const edm::ParameterSet& ps) :
  hlt_eventToken_(consumes<trigger::TriggerEvent>(ps.getParameter<edm::InputTag>("TriggerEvent"))),
...
void
HLTFilter::beginRun(const edm::Run& r,
                    const edm::EventSetup& es)
{
  bool changed(true);
  if (hltConfig_.init(r, es,  hltProcessName_, changed)) {
  } else {
    throw cms::Exception("config extraction failure with processName -> "+hltProcessName_);
  }
}

bool
HLTFilter::filter(edm::Event& e,
                  const edm::EventSetup& es)
{
  using namespace std;
  ...
  {
    const vector<string> matchedPaths(hltConfig_.restoreVersion(allTriggerPaths, p));
    if (matchedPaths.size() == 0) {
      throw cms::Exception("Could not find matched full trigger path -> "+p);
    }
