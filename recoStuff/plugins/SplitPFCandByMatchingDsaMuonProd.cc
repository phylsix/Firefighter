...
SplitPFCandByMatchingDsaMuonProd::SplitPFCandByMatchingDsaMuonProd(const edm::ParameterSet& iC) :
  srcToken_(consumes<reco::PFCandidateCollection>(iC.getParameter<edm::InputTag>("src"))),
  matchedToken_(consumes<reco::PFCandidateCollection>(iC.getParameter<edm::InputTag>("matched"))),
  srcCut_(iC.getParameter<std::string>("srcPreselection")),
  matcherByTk_(iC.getParameterSet("matcherByTkParams")),
  matcherByMu_(iC.getParameterSet("matcherByMuParams"), consumesCollector())
{
                                          ...
                                          const edm::EventSetup& iSetup)
{
  ...
  for (size_t isrc(0); isrc!=srcHdl_->size(); ++isrc)
  {
    ...
    if (!srcCut_(srcCand)) continue;
    if (srcCand.particleId() != reco::PFCandidate::ParticleType::mu) continue;
