...
const float TrackerBoundsRadius     = 740;  // 112;
const float TrackerBoundsHalfLength = 960;  // 273.5;
bool
insideTrackerBounds( const GlobalPoint& point ) {
...
ff::SequentialVertexFitter<N>::SequentialVertexFitter(
...
template <unsigned int N>
void
ff::SequentialVertexFitter<N>::readParameters() {
...
template <unsigned int N>
void
ff::SequentialVertexFitter<N>::setDefaultParameters() {
...
template <unsigned int N>
CachingVertex<N>
ff::SequentialVertexFitter<N>::vertex(
 ...
 * The specified LinearizationPointFinder will be used to find the linearization
 * point.
 */
template <unsigned int N>
CachingVertex<N>
...
//
template <unsigned int N>
vector<typename ff::SequentialVertexFitter<N>::RefCountedVertexTrack>
ff::SequentialVertexFitter<N>::linearizeTracks(
    const std::vector<reco::TransientTrack>& tracks,
  ...
  for ( vector<reco::TransientTrack>::const_iterator i = tracks.begin();
    ...
    RefCountedVertexTrack vTrData =
        theVTrackFactory.vertexTrack( lTrData, state );
...
template <unsigned int N>
vector<typename ff::SequentialVertexFitter<N>::RefCountedVertexTrack>
ff::SequentialVertexFitter<N>::reLinearizeTracks(
    const std::vector<RefCountedVertexTrack>& tracks,
    const VertexState                         state ) const {
  GlobalPoint                        linP = state.position();
  std::vector<RefCountedVertexTrack> finalTracks;
...
template <unsigned int N>
CachingVertex<N>
ff::SequentialVertexFitter<N>::fit(
    const std::vector<RefCountedVertexTrack>& tracks,
  ...
  CachingVertex<N> returnVertex( priorVertexPosition, priorVertexError,
                                 initialTracks, 0 );
  if ( withPrior ) {
  ...
  do {
    ...
    if ( !validVertex ) {
      ...
      GlobalError                 error( we * 10000 );
      fVertex =
          CachingVertex<N>( GlobalPoint( 0, 0, 0 ), error, initialTracks, 0 );
    }
  ...
  if ( !validVertex ) {
    LogDebug( "recoStuff/SequentialVertexFitter" )
        << "Fitted position is invalid (out of tracker bounds or has NaN). "
           "Returned vertex is invalid\n";
    return CachingVertex<N>();  // return invalid vertex
  }

  if ( step >= theMaxStep ) {
    LogDebug( "recoStuff/SequentialVertexFitter" )
        << "The maximum number of steps has been exceeded. Returned vertex is "
           "invalid\n";
    return CachingVertex<N>();  // return invalid vertex
  }
...
template class ff::SequentialVertexFitter<5>;
template class ff::SequentialVertexFitter<6>;
