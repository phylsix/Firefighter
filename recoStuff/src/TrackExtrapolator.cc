...
ff::TrackExtrapolator::TrackExtrapolator()
  : radius_(0.), absZ_(0.), bField_(nullptr), propagator_(nullptr), geometry_(nullptr)
{}
...
ff::TrackExtrapolator::TrackExtrapolator(float radius, float absZ, const edm::EventSetup& es)
{
...
ff::TrackExtrapolator::propagate(const FreeTrajectoryState& fts) const
{
  TrajectoryStateOnSurface propagatedInfo = propagator_->propagate(
    fts,
    *Cylinder::build(radius_, Surface::PositionType(0,0,0), Surface::RotationType())
  );

  if (!propagatedInfo.isValid()) return TrajectoryStateOnSurface();

  if (propagatedInfo.globalPosition().z()>absZ_)
  {
    propagatedInfo = propagator_->propagate(
      fts,
      *Plane::build(Surface::PositionType(0,0,absZ_), Surface::RotationType())
    );
...
ff::TrackExtrapolator::propagate(const reco::GenParticle& particle) const
{
  ...
  GlobalVector momentum = GlobalVector(particle.px(), particle.py(), particle.pz());
