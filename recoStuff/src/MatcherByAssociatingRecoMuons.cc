...
ff::MatcherByAssociatingRecoMuons::MatcherByAssociatingRecoMuons(const edm::ParameterSet& ps,
                                                                 edm::ConsumesCollector&& cc) :
  recoMuon_token_(cc.consumes<reco::MuonCollection>(ps.getParameter<edm::InputTag>("recoMuons"))),
...
ff::MatcherByAssociatingRecoMuons::init(const edm::Event& e)
{
  ...
  for (const auto& muon : *recoMuon_h)
  {
    ...
    if (innerTk.isNull() or outerTk.isNull()) continue;
...
ff::MatcherByAssociatingRecoMuons::match(const std::vector<reco::PFCandidateRef>& srcCol,
                                         const std::vector<reco::PFCandidateRef>& matchCol) const
{
  ...
  for (const auto& srcCand : sortedSrcCol)
  {
    ...
    if (recoMuonTrackLink_.find(srcTk) == recoMuonTrackLink_.end()) continue;
    ...
    for (const auto& matCand : matchCol)
    {
      ...
      bool isOverlapped = checkOverlap(associatedOuterTk, matTk, minHitsOverlapRatio_);
...
{
  if (src.isNull() or comp.isNull()) return false;

  const reco::HitPattern& srcHp = src->hitPattern();
  const reco::HitPattern& compHp = comp->hitPattern();

  std::vector<uint16_t> srcHits{};
  for (int i(0); i!=srcHp.numberOfAllHits(reco::HitPattern::TRACK_HITS); ++i)
    srcHits.push_back(srcHp.getHitPattern(reco::HitPattern::TRACK_HITS, i));
  
  std::vector<uint16_t> compHits{};
  for (int i(0); i!=compHp.numberOfAllHits(reco::HitPattern::TRACK_HITS); ++i)
    compHits.push_back(compHp.getHitPattern(reco::HitPattern::TRACK_HITS, i));  
...
float
ff::MatcherByAssociatingRecoMuons::getPtRel(const reco::TrackRef& src,
                                            const reco::TrackRef& comp) const
{
  if (src.isNull() or comp.isNull()) return NAN;
