...
// Based on the R.Fruhwirth et al Computer Physics Communications 96 (1996)
// 189-208
template <unsigned int N>
CachingVertex<N>
ff::KalmanVertexUpdator<N>::update( const CachingVertex<N>&     oldVertex,
  ...
  VertexState newVertexState = positionUpdate(
      oldVertex.vertexState(), track->linearizedTrack(), weight, sign );
  ...
  std::pair<bool, double> chi2P =
      chi2Increment( oldVertex.vertexState(), newVertexState,
  ...
  if ( sign > 0 ) {
    ...
    if ( pos != newVertexTracks.end() ) {
      ...
      std::cout << "KalmanVertexUpdator::Unable to find requested track in the "
                   "current vertex"
                << std::endl;
      throw VertexException(
          "KalmanVertexUpdator::Unable to find requested track in the current "
          "vertex" );
    }
  }

  if ( oldVertex.hasPrior() ) {
    return CachingVertex<N>( oldVertex.priorVertexState(), newVertexState,
                             newVertexTracks, chi1 );
  } else {
...
template <unsigned int N>
CachingVertex<N>
ff::KalmanVertexUpdator<N>::add( const CachingVertex<N>&     oldVertex,
...
template <unsigned int N>
CachingVertex<N>
ff::KalmanVertexUpdator<N>::remove( const CachingVertex<N>&     oldVertex,
                                    const RefCountedVertexTrack track ) const {
  float weight = track->weight();
  return update( oldVertex, track, weight, -1 );
}
...
template <unsigned int N>
VertexState
ff::KalmanVertexUpdator<N>::positionUpdate(
    const VertexState&                   oldVertex,
  ...
  ParticleKinematicLinearizedTrackState* linTkState =
      dynamic_cast<ParticleKinematicLinearizedTrackState*>(
          linearizedTrack.get() );
  if ( linTkState != nullptr ) {
    TrackKinematicStatePropagator propagator;
    if ( !propagator
              .propagateToTheTransversePCA(
                  linTkState->particle()->currentState(),
                  linTkState->linearizationPoint() )
              .isValid() ) {
      edm::LogWarning( "KalmanVertexUpdator" )
          << "KinematicState after propagation is invalid. An invalid vertex "
             "will be returned.";
      return VertexState();
    }
  }
  ...
  if ( error != 0 ) {
    edm::LogWarning( "KalmanVertexUpdator" )
        << "predictedState error matrix inversion failed. An invalid vertex "
           "will be returned.";
    return VertexState();
  }
  ...
  AlgebraicSymMatrixNN gB =
      trackParametersWeight -
      ROOT::Math::Similarity( trackParametersWeight,
                              ROOT::Math::Similarity( b, s ) );

  // Getting the new covariance matrix of the vertex.
  AlgebraicSymMatrix33 newVertexWeight =
      oldVertex.weight().matrix() +
...
ff::KalmanVertexUpdator<N>::chi2Increment(
    const VertexState&                   oldVertex,
  ...
  AlgebraicVectorM newTrackMomentumP =
      s * ROOT::Math::Transpose( b ) * trackParametersWeight * vv;

  //   AlgebraicVectorN rtp = ( theResidual +  a * newVertexPositionV + b *
  //   newTrackMomentumP);

  AlgebraicVectorN parameterResiduals = vv - b * newTrackMomentumP;
  linearizedTrack->checkParameters( parameterResiduals );

  double chi2 = weight * ROOT::Math::Similarity( parameterResiduals,
                                                 trackParametersWeight );
...
template class ff::KalmanVertexUpdator<5>;
template class ff::KalmanVertexUpdator<6>;