...
CachingVertex<N> ff::KalmanVertexUpdator<N>::update(const  CachingVertex<N> & oldVertex,
	const RefCountedVertexTrack track, float weight, int sign) const
{
  ...
  else
  {
    ...
    typename std::vector<RefCountedVertexTrack>::iterator pos = find(newVertexTracks.begin(), newVertexTracks.end(), track);
...
template <unsigned int N>
CachingVertex<N> ff::KalmanVertexUpdator<N>::add(const CachingVertex<N> & oldVertex, 
...
template <unsigned int N>
CachingVertex<N> ff::KalmanVertexUpdator<N>::remove(const CachingVertex<N> & oldVertex, 
    const RefCountedVertexTrack track) const
{
  float weight = track->weight();
  return update(oldVertex,track,weight,-1);
...
template <unsigned int N>
VertexState 
ff::KalmanVertexUpdator<N>::positionUpdate (const VertexState & oldVertex,
	 ...
	 const float weight, int sign) const
{
  ...
  ParticleKinematicLinearizedTrackState* linTkState
    = dynamic_cast<ParticleKinematicLinearizedTrackState*>(linearizedTrack.get());
  if (linTkState!=nullptr) {
    TrackKinematicStatePropagator propagator;
    if (!propagator.propagateToTheTransversePCA(linTkState->particle()->currentState(),
                                                linTkState->linearizationPoint()).isValid())
    {
      edm::LogWarning("KalmanVertexUpdator") << "KinematicState after propagation is invalid. An invalid vertex will be returned.";
      return VertexState();
    }
  }
...
template <unsigned int N>
std::pair <bool, double>  ff::KalmanVertexUpdator<N>::chi2Increment(const VertexState & oldVertex, 
