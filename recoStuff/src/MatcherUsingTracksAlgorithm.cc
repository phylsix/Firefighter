...
ff::MatcherUsingTracksAlgorithm::MatcherUsingTracksAlgorithm(
    const edm::ParameterSet& iConfig )
    : whichTrack1_( None ),
      whichTrack2_( None ),
      whichState1_( AtVertex ),
      whichState2_( AtVertex ),
      srcCut_( iConfig.existsAs<std::string>( "srcPreselection" )
                   ? iConfig.getParameter<std::string>( "srcPreselection" )
                   : "" ),
      matchedCut_(
          iConfig.existsAs<std::string>( "matchedPreselection" )
              ? iConfig.getParameter<std::string>( "matchedPreselection" )
              : "" ),
      requireSameCharge_(
          iConfig.existsAs<bool>( "requireSameCharge" )
              ? iConfig.getParameter<bool>( "requireSameCharge" )
              : false ) {
  ...
  if ( algo == "byTrackRef" ) {
    ...
    throw cms::Exception( "Configuration" )
        << "Value '" << algo << "' for algorithm not yet implemented.\n";
  ...
  if ( algo_ == ByTrackRef ) {
  ...
  } else if ( algo_ == ByPropagatingSrc || algo_ == ByPropagatingMatched ||
              algo_ == ByDirectComparison ) {
    // read matching cuts
    maxLocalPosDiff_      = iConfig.getParameter<double>( "maxDeltaLocalPos" );
    maxGlobalMomDeltaR_   = iConfig.getParameter<double>( "maxDeltaR" );
    maxGlobalMomDeltaEta_ = iConfig.existsAs<double>( "maxDeltaEta" )
                                ? iConfig.getParameter<double>( "maxDeltaEta" )
                                : maxGlobalMomDeltaR_;
    maxGlobalMomDeltaPhi_ = iConfig.existsAs<double>( "maxDeltaPhi" )
    ...
    if ( algo_ == ByPropagatingSrc ) {
      if ( whichTrack2_ == None || whichState2_ == AtVertex ) {
        ...
        // throw cms::Exception("Configuration") << "Destination track must be
        // non-null, and state must not be 'AtVertex' (not yet).\n";
      }
      ...
      if ( firstAtVertex ) {
        if ( !secAtVertex )
          throw cms::Exception( "Configuration" )
              << "When using 'byDirectComparison' with 'src' at vertex (or "
                 "None), 'matched' must be at vertex too.\n";
      } else {
        if ( secAtVertex )
          throw cms::Exception( "Configuration" )
              << "When using 'byDirectComparison' with 'src' not at vertex, "
                 "'matched' can't be at vertex or None.\n";
        if ( whichState1_ != whichState2_ )
          throw cms::Exception( "Configuration" )
              << "You can't use 'byDirectComparison' with non-matching "
                 "states.\n";
      }
    }

    useChi2_ = iConfig.existsAs<bool>( "computeChi2" )
                   ? iConfig.getParameter<bool>( "computeChi2" )
                   : false;
    if ( useChi2_ ) {
      if ( whichTrack1_ == None && whichTrack2_ == None )
        throw cms::Exception( "Configuration" )
      ...
      if ( algo_ == ByPropagatingSrc || algo_ == ByPropagatingMatched ) {
        ...
        if ( algo_ == ByDirectComparison ) {
          std::string choice =
              iConfig.getParameter<std::string>( "chi2MomentumForDxy" );
          if ( choice == "src" )
...
void
ff::MatcherUsingTracksAlgorithm::getConf( const edm::ParameterSet& iConfig,
                                          const std::string&       whatFor,
                                          WhichTrack&              whichTrack,
                                          WhichState& whichState ) {
  std::string s_whichTrack =
      iConfig.getParameter<std::string>( whatFor + "Track" );
  ...
  if ( ( whichTrack != None ) && ( algo_ != ByTrackRef ) ) {
    std::string s_whichState =
        iConfig.getParameter<std::string>( whatFor + "State" );
...
/// For matches not by ref, it will update deltaR, deltaLocalPos and deltaPtRel
/// if the match suceeded
bool
ff::MatcherUsingTracksAlgorithm::match( const reco::Candidate& c1,
  ...
  switch ( algo_ ) {
      ...
      if ( t1.isNonnull() ) {
        ...
        if ( t1.id() != t2.id() ) {
          edm::LogWarning( "MatcherUsingTracksAlgorithm" )
              << "Trying to match by reference tracks coming from different "
                 "collections.\n";
        }
      }
    }
    case ByPropagatingSrc: {
      FreeTrajectoryState start =
          startingState( c1, whichTrack1_, whichState1_ );
      TrajectoryStateOnSurface target =
          targetState( c2, whichTrack2_, whichState2_ );
      return matchWithPropagation( start, target, deltR, deltEta, deltPhi,
                                   deltaLocalPos, deltaPtRel, chi2 );
    }
    case ByPropagatingMatched: {
      FreeTrajectoryState start =
          startingState( c2, whichTrack2_, whichState2_ );
      TrajectoryStateOnSurface target =
          targetState( c1, whichTrack1_, whichState1_ );
      return matchWithPropagation( start, target, deltR, deltEta, deltPhi,
                                   deltaLocalPos, deltaPtRel, chi2 );
    }
    ...
    case ByDirectComparison: {
      FreeTrajectoryState start =
          startingState( c1, whichTrack1_, whichState1_ );
      FreeTrajectoryState otherstart =
          startingState( c2, whichTrack2_, whichState2_ );
      return matchByDirectComparison( start, otherstart, deltR, deltEta,
                                      deltPhi, deltaLocalPos, deltaPtRel,
                                      chi2 );
    }
  }
  return false;
...
/// For matches not by ref, it will update deltaR, deltaLocalPos and deltaPtRel
/// if the match suceeded Returns -1 if the match fails
int
ff::MatcherUsingTracksAlgorithm::match( const reco::Candidate&            c1,
  ...
  if ( algo_ == ByPropagatingSrc || algo_ == ByPropagatingSrcTSCP ||
       algo_ == ByPropagatingMatchedTSCP || algo_ == ByDirectComparison ) {
  ...
  for ( it = c2s.begin(), ed = c2s.end(), i = 0; it != ed; ++it, ++i ) {
    ...
    switch ( algo_ ) {
      ...
      case ByPropagatingMatched: {
        ...
        if ( matchWithPropagation( start, target, deltR, deltEta, deltPhi,
                                   deltaLocalPos, deltaPtRel, chi2 ) ) {
          match = i;
        }
      } break;
      case ByDirectComparison: {
        FreeTrajectoryState otherstart =
            startingState( *it, whichTrack2_, whichState2_ );
        if ( matchByDirectComparison( start, otherstart, deltR, deltEta,
                                      deltPhi, deltaLocalPos, deltaPtRel,
                                      chi2 ) ) {
          match = i;
        }
      ...
      case ByPropagatingSrcTSCP: {
        FreeTrajectoryState otherstart =
            startingState( *it, whichTrack2_, whichState2_ );
        if ( matchWithPropagation( start, otherstart, deltR, deltEta, deltPhi,
                                   deltaLocalPos, deltaPtRel, chi2 ) ) {
          match = i;
        }
      } break;
      case ByPropagatingMatchedTSCP: {
        FreeTrajectoryState otherstart =
            startingState( *it, whichTrack2_, whichState2_ );
        if ( matchWithPropagation( otherstart, start, deltR, deltEta, deltPhi,
                                   deltaLocalPos, deltaPtRel, chi2 ) ) {
          match = i;
        }
      } break;
...
void
ff::MatcherUsingTracksAlgorithm::init( const edm::EventSetup& iSetup ) {
...
// reco::TrackRef
// MatcherUsingTracksAlgorithm::getTrack(const reco::Candidate &reco, WhichTrack
// whichTrack) const {
//     reco::TrackRef tk;
//     const reco::RecoCandidate *rc = dynamic_cast<const reco::RecoCandidate
//     *>(&reco); if (rc == nullptr) throw cms::Exception("Invalid Data") <<
...
reco::TrackRef
ff::MatcherUsingTracksAlgorithm::getTrack( const reco::Candidate& reco,
                                           WhichTrack whichTrack ) const {
  reco::TrackRef             tk;
  ...
  const reco::PFCandidate* pfc =
      dynamic_cast<const reco::PFCandidate*>( &reco );
  if ( rc == nullptr && pfc == nullptr )
    throw cms::Exception( "Invalid Data" )
        << "Input object is not a RecoCandidate, nor a PFCandidate.\n";
  tk = pfc->trackRef();
  return tk;
}

FreeTrajectoryState
ff::MatcherUsingTracksAlgorithm::startingState( const reco::Candidate& reco,
                                                WhichTrack whichTrack,
  ...
  if ( whichTrack != None ) {
    ...
    if ( tk.isNull() ) {
      ...
      switch ( whichState ) {
        case AtVertex:
          ret = trajectoryStateTransform::initialFreeState(
              *tk, magfield_.product() );
          break;
        case Innermost:
          ret = trajectoryStateTransform::innerFreeState( *tk,
                                                          magfield_.product() );
          break;
        case Outermost:
          ret = trajectoryStateTransform::outerFreeState( *tk,
                                                          magfield_.product() );
          break;
      }
    }
...
ff::MatcherUsingTracksAlgorithm::targetState( const reco::Candidate& reco,
                                              WhichTrack             whichTrack,
                                              WhichState whichState ) const {
  ...
  if ( tk.isNonnull() ) {
    switch ( whichState ) {
      case Innermost:
        ret = trajectoryStateTransform::innerStateOnSurface(
            *tk, *geometry_, magfield_.product() );
        break;
      case Outermost:
        ret = trajectoryStateTransform::outerStateOnSurface(
            *tk, *geometry_, magfield_.product() );
        break;
      default:
        break;  // just to make gcc happy
...
bool
ff::MatcherUsingTracksAlgorithm::matchWithPropagation(
    const FreeTrajectoryState&      start,
  ...
  TrajectoryStateOnSurface tsos =
      propagator_->propagate( start, target.surface() );

  bool isBest = false;
  if ( tsos.isValid() ) {
    float thisLocalPosDiff =
        ( tsos.localPosition() - target.localPosition() ).mag();
    float thisGlobalMomDeltaR =
        deltaR( tsos.globalMomentum(), target.globalMomentum() );
    float thisGlobalMomDeltaPhi = fabs( deltaPhi(
        tsos.globalMomentum().barePhi(), target.globalMomentum().barePhi() ) );
    float thisGlobalMomDeltaEta =
        fabs( tsos.globalMomentum().eta() - target.globalMomentum().eta() );
    float thisGlobalDPtRel =
        ( tsos.globalMomentum().perp() - target.globalMomentum().perp() ) /
        target.globalMomentum().perp();
    // std::cout<<"[matchWithPropagation] "<<thisLocalPosDiff<<",
    // "<<thisGlobalMomDeltaR<<", "<<thisGlobalDPtRel<<std::endl;
    if ( ( thisLocalPosDiff < maxLocalPosDiff_ ) &&
      ...
      float thisChi2 =
          useChi2_ ? getChi2( target, tsos, chi2DiagonalOnly_, chi2UseVertex_ )
                   : 0;
...
ff::MatcherUsingTracksAlgorithm::matchWithPropagation(
  ...
  /*2.2.X*/ try {
    ...
    float thisGlobalMomDeltaPhi = fabs(
        deltaPhi( tscp.momentum().barePhi(), target.momentum().barePhi() ) );
    float thisGlobalMomDeltaEta =
        fabs( tscp.momentum().eta() - target.momentum().eta() );
    float thisGlobalDPtRel =
        ( tscp.momentum().perp() - target.momentum().perp() ) /
    ...
    if ( ( thisLocalPosDiff < maxLocalPosDiff_ ) &&
         ...
         ( fabs( thisGlobalDPtRel ) < maxGlobalDPtRel_ ) ) {
      float thisChi2 =
          useChi2_ ? getChi2( target, tscp, chi2DiagonalOnly_, chi2UseVertex_ )
                   : 0;
      if ( thisChi2 >= maxChi2_ )
        return false;
      switch ( sortBy_ ) {
...
bool
ff::MatcherUsingTracksAlgorithm::matchByDirectComparison(
    const FreeTrajectoryState& start,
    const FreeTrajectoryState& target,
    float&                     lastDeltaR,
    float&                     lastDeltaEta,
  ...
  float thisGlobalMomDeltaPhi = fabs(
      deltaPhi( start.momentum().barePhi(), target.momentum().barePhi() ) );
  float thisGlobalMomDeltaEta =
      fabs( start.momentum().eta() - target.momentum().eta() );
  float thisGlobalDPtRel =
      ( start.momentum().perp() - target.momentum().perp() ) /
      target.momentum().perp();
  ...
  if ( ( thisLocalPosDiff < maxLocalPosDiff_ ) &&
       ...
       ( thisGlobalMomDeltaPhi < maxGlobalMomDeltaPhi_ ) &&
       ( fabs( thisGlobalDPtRel ) < maxGlobalDPtRel_ ) ) {
    float thisChi2 = useChi2_ ? getChi2( start, target, chi2DiagonalOnly_,
                                         chi2UseVertex_, chi2FirstMomentum_ )
                              : 0;
    if ( thisChi2 >= maxChi2_ )
      return false;
    switch ( sortBy_ ) {
...
double
ff::MatcherUsingTracksAlgorithm::getChi2( const FreeTrajectoryState& start,
  ...
  double       dsz = ( x1.z() - x2.z() ) * pt / pm -
               ( ( x1.x() - x2.x() ) * p.x() + ( x1.y() - x2.y() ) * p.y() ) /
                   pt * p.z() / pm;
  double dxy =
      ( -( x1.x() - x2.x() ) * p.y() + ( x1.y() - x2.y() ) * p.x() ) / pt;
  AlgebraicVector5 diff( start.charge() / p1.mag() - other.charge() / p2.mag(),
                         p1.theta() - p2.theta(),
                         ( p1.phi() - p2.phi() ).value(), dxy, dsz );
  return ROOT::Math::Similarity( diff, cov );
}

double
ff::MatcherUsingTracksAlgorithm::getChi2(
    const FreeTrajectoryState&           start,
  ...
  cropAndInvert( cov, diagonalOnly, !useVertex );
  AlgebraicVector5 pgpar1 = PerigeeConversions::ftsToPerigeeParameters(
                                start, other.referencePoint(), pt )
                                .vector();
...
double
ff::MatcherUsingTracksAlgorithm::getChi2( const TrajectoryStateOnSurface& start,
  ...
  AlgebraicVector5 diff( start.localParameters().mixedFormatVector() -
                         other.localParameters().mixedFormatVector() );
  return ROOT::Math::Similarity( diff, cov );
}

void
ff::MatcherUsingTracksAlgorithm::cropAndInvert( AlgebraicSymMatrix55& cov,
                                                bool diagonalOnly,
                                                bool top3by3only ) {
  if ( !top3by3only ) {
    if ( diagonalOnly ) {
    ...
    // get 3x3 covariance
    AlgebraicSymMatrix33 momCov =
        cov.Sub<AlgebraicSymMatrix33>( 0, 0 );  // get 3x3 matrix
    if ( diagonalOnly ) {
      momCov( 0, 1 ) = 0;
      momCov( 0, 2 ) = 0;
      momCov( 1, 2 ) = 0;
