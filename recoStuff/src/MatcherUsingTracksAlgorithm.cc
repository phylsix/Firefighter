...
ff::MatcherUsingTracksAlgorithm::MatcherUsingTracksAlgorithm(const edm::ParameterSet & iConfig) :
...
void 
ff::MatcherUsingTracksAlgorithm::getConf(const edm::ParameterSet & iConfig, const std::string &whatFor, WhichTrack &whichTrack, WhichState &whichState) {
...
bool 
ff::MatcherUsingTracksAlgorithm::match(const reco::Candidate &c1, const reco::Candidate &c2, float &deltR, float &deltEta, float &deltPhi, float &deltaLocalPos, float &deltaPtRel, float &chi2) const {
...
int 
ff::MatcherUsingTracksAlgorithm::match(const reco::Candidate &c1, const edm::View<reco::Candidate> &c2s, float &deltR, float &deltEta, float &deltPhi, float &deltaLocalPos, float &deltaPtRel, float &chi2) const {
...
reco::TrackRef
ff::MatcherUsingTracksAlgorithm::getTrack(const reco::Candidate &reco, WhichTrack whichTrack) const {
    reco::TrackRef tk;
    const reco::RecoCandidate *rc = dynamic_cast<const reco::RecoCandidate *>(&reco);
    if (rc != nullptr)
    {
    ...
    const reco::PFCandidate *pfc = dynamic_cast<const reco::PFCandidate *>(&reco);
    if (rc==nullptr && pfc==nullptr)
      throw cms::Exception("Invalid Data") << "Input object is not a RecoCandidate, nor a PFCandidate.\n";
    tk = pfc->trackRef();
    return tk;
}

FreeTrajectoryState 
ff::MatcherUsingTracksAlgorithm::startingState(const reco::Candidate &reco, WhichTrack whichTrack, WhichState whichState) const {
...
ff::MatcherUsingTracksAlgorithm::targetState(const reco::Candidate &reco, WhichTrack whichTrack, WhichState whichState) const {
...
bool
ff::MatcherUsingTracksAlgorithm::matchWithPropagation(const FreeTrajectoryState &start, 
                                                      ...
                                                      float &lastChi2) const 
{
    ...
    if (tsos.isValid()) {
        ...
        // std::cout<<"[matchWithPropagation] "<<thisLocalPosDiff<<", "<<thisGlobalMomDeltaR<<", "<<thisGlobalDPtRel<<std::endl;
...
bool
ff::MatcherUsingTracksAlgorithm::matchByDirectComparison(const FreeTrajectoryState &start, 
                                                         const FreeTrajectoryState &target, 
                                                         float &lastDeltaR,
                                                         float &lastDeltaEta,
                                                         float &lastDeltaPhi, 
...
double
ff::MatcherUsingTracksAlgorithm::getChi2(const FreeTrajectoryState &start, const FreeTrajectoryState &other, bool diagonalOnly, bool useVertex, bool useFirstMomentum)
{
...
double
ff::MatcherUsingTracksAlgorithm::getChi2(const FreeTrajectoryState &start, const TrajectoryStateClosestToPoint &other, bool diagonalOnly, bool useVertex)
{
    if (!start.hasError() && !other.hasError()) throw cms::Exception("LogicError") << "At least one of the two states must have errors to make chi2s.\n";
...
double
ff::MatcherUsingTracksAlgorithm::getChi2(const TrajectoryStateOnSurface &start, const TrajectoryStateOnSurface &other, bool diagonalOnly, bool usePosition)
{
    if (!start.hasError() && !other.hasError()) throw cms::Exception("LogicError") << "At least one of the two states must have errors to make chi2s.\n";
    AlgebraicSymMatrix55 cov;
...
void
ff::MatcherUsingTracksAlgorithm::cropAndInvert(AlgebraicSymMatrix55 &cov, bool diagonalOnly, bool top3by3only)
{
    if (!top3by3only) {
        if (diagonalOnly) {
