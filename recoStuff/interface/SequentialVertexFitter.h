...
class SequentialVertexFitter : public VertexFitter<N> {
 public:
  ...
  /**
   * Method returning the fitted vertex, from a container of
   * reco::TransientTracks. The linearization point will be searched with the
   * given LP finder. No prior vertex position will be used in the vertex fit.
   * \param tracks The container of RecTracks to fit.
   * \return The fitted vertex
   */
  CachingVertex<N> vertex(
   ...
   */
  CachingVertex<N> vertex(
      const std::vector<RefCountedVertexTrack>& tracks ) const override;
  ...
  /** Fit vertex out of a set of RecTracks. Uses the specified linearization
   * point.
   */
   ...
   * will be created with the last estimate of the vertex as linearization
   * point. In case a non-sero error is given, the position and error of the
 ...
 private:
   ...
   * \param tracks The original container of VertexTracks, from which the
   *RecTracks will be extracted. \param seed The seed to use for the
   *VertexTracks. This position will also be used as the new linearization
   *point. \return The container of VertexTracks which are to be used in the
   *next fit.
   */
  ...
  inline bool hasNan( const GlobalPoint& point ) const {
    using namespace std;
    return ( edm::isNotFinite( point.x() ) || edm::isNotFinite( point.y() ) ||
             edm::isNotFinite( point.z() ) );
  }
