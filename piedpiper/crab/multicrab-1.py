...
def main():
    ...
    # load config
    multiconf = yaml.load(open('multicrabConfig-1.yml').read())
    ...
    for ds in inputdatasets:
        ...
        cmd = 'crab submit -c crabConfig-1.py'

        if verbose:
            print(">> ", os.path.join(BASEDIR, 'crab/config-1.yml'))
            print(singlecrabConfigRECO.format(DS=ds, NT=nametag))
            print(cmd)
            print('-----------------------------------------------------')

        if doCmd:
            with open(os.path.join(BASEDIR, 'crab/config-1.yml'), 'w') as conf:
    ...
    if undonelist:
        with open('unsubmitted-1.yml', 'w') as outf:
            yaml.dump({'gensimdatasets': undonelist}, outf, default_flow_style=False)
