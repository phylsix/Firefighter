...
# JpsiToMuMu
fn = 'root://cmsxrootd.fnal.gov//store/mc/RunIIAutumn18DRPremix/JpsiToMuMu_JpsiPt8_TuneCP5_13TeV-pythia8/AODSIM/102X_upgrade2018_realistic_v15-v1/1110000/50BAD636-D0A9-1249-8D41-85F3FD348064.root'
events = Events(fn)
...
for i, event in enumerate(events, 1):
    ...
    if i%1000 == 1: print("Event : {}".format(i))
    # if i>10: break
    ...
    muCol_ = sorted([g for g in genp if g.isLastCopy() and abs(g.pdgId())==13], key=lambda p: p.pt(), reverse=True)[:2]
...
ax.hist(np.array(res_pt[443]), bins=50, range=[0, 50], histtype='step', normed=True, label='$J/\psi$')
ax.hist(np.array(res_pt[13] ), bins=50, range=[0, 50], histtype='step', normed=True, label='$\mu$')
...
ax.hist(np.array(res_eta[443]), bins=50, range=[-6, 6], histtype='step', normed=True, label='$J/\psi$')
ax.hist(np.array(res_eta[13] ), bins=50, range=[-6, 6], histtype='step', normed=True, label='$\mu$')
...
ax.hist(np.array(res_phi[443]), bins=50, range=[-3.142, 3.142], histtype='step', normed=True, label='$J/\psi$')
