...
# WWZ
fn = 'root://cmsxrootd.fnal.gov//store/mc/RunIIAutumn18DRPremix/WWZ_TuneCP5_13TeV-amcatnlo-pythia8/AODSIM/102X_upgrade2018_realistic_v15_ext1-v2/80000/29FAA129-6A94-6144-BBEA-246829F24253.root'
events = Events(fn)
...
for i, event in enumerate(events, 1):
    ...
    muCol_ = sorted([g for g in genp if g.isLastCopy() and abs(g.pdgId())==13], key=lambda p: p.pt(), reverse=True)[:6]
    elCol_ = sorted([g for g in genp if g.isLastCopy() and abs(g.pdgId())==11], key=lambda p: p.pt(), reverse=True)[:6]
