...
# ZZTo2L2Nu
fn = 'root://cmsxrootd.fnal.gov//store/mc/RunIIAutumn18DRPremix/ZZTo2L2Nu_TuneCP5_13TeV_powheg_pythia8/AODSIM/102X_upgrade2018_realistic_v15_ext1-v2/20000/FDF58147-9D2C-804E-9865-77691C66636C.root'
events = Events(fn)
...
for i, event in enumerate(events, 1):
    ...
    if i%1000 == 1: print("ZZTo2L2Nu Event : {}".format(i))
    # if i>30: break
    ...
    elCol_ = sorted([g for g in genp if g.isLastCopy() and abs(g.pdgId())==11], key=lambda p: p.pt(), reverse=True)[:2]
    ...
    if len(muCol_)>=2: res_dR[13].append( delta_r(muCol_[0], muCol_[1]) )
    if len(elCol_)>=2: res_dR[11].append( delta_r(elCol_[0], elCol_[1]) )
    if i<10:
        print('-'*(6*3 + 5 + 12*3))
    ...
    if len(recoMu)>=2: recoMu_dR.append( delta_r(recoMu[0], recoMu[1]) )
...
ax.hist(np.array(recoMu_dR), bins=50, range=[0, 8], histtype='step', normed=True, label='reco $\mu$')
