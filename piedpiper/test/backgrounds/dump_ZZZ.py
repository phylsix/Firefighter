...
# ZZZ
fn = 'root://cmsxrootd.fnal.gov//store/mc/RunIIAutumn18DRPremix/ZZZ_TuneCP5_13TeV-amcatnlo-pythia8/AODSIM/102X_upgrade2018_realistic_v15_ext1-v2/100000/EE36B8E0-B9E2-7943-9DD4-DBD9D0573AD7.root'
events = Events(fn)
...
for i, event in enumerate(events, 1):
    ...
    muCol_ = sorted([g for g in genp if g.isLastCopy() and abs(
        g.pdgId()) == 13], key=lambda p: p.pt(), reverse=True)[:6]
    elCol_ = sorted([g for g in genp if g.isLastCopy() and abs(
        g.pdgId()) == 11], key=lambda p: p.pt(), reverse=True)[:6]
