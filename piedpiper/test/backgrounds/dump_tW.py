...
# tW
fn = 'root://cmsxrootd.fnal.gov//store/mc/RunIIAutumn18DRPremix/ST_tW_top_5f_inclusiveDecays_TuneCP5_13TeV-powheg-pythia8/AODSIM/102X_upgrade2018_realistic_v15_ext1-v1/20000/FFFBE773-3FE4-3142-B20C-A8986ECBE6CB.root'
events = Events(fn)
...
for i, event in enumerate(events, 1):
    ...
    muCol_ = sorted([g for g in genp if g.isLastCopy() and abs(
        g.pdgId()) == 13], key=lambda p: p.pt(), reverse=True)[:4]
    elCol_ = sorted([g for g in genp if g.isLastCopy() and abs(
        g.pdgId()) == 11], key=lambda p: p.pt(), reverse=True)[:4]
...
ax.hist(np.array(res_pt[6]), bins=50, range=[0, 500],
        histtype='step', normed=True, label='gen t')
ax.hist(np.array(res_pt[24]), bins=50, range=[0, 500],
        histtype='step', normed=True, label='gen W')
...
ax.hist(np.array(recoMu_pt), bins=50, range=[
        0, 500], histtype='step', normed=True, label='reco $\mu$')
...
ax.hist(np.array(res_eta[6]), bins=50, range=[-6, 6],
        histtype='step', normed=True, label='gen t')
ax.hist(np.array(res_eta[24]), bins=50, range=[-6, 6],
        histtype='step', normed=True, label='gen W')
ax.hist(np.array(res_eta[13]), bins=50, range=[-6, 6],
        histtype='step', normed=True, label='gen $\mu$')
ax.hist(np.array(res_eta[11]), bins=50, range=[-6, 6],
        histtype='step', normed=True, label='gen e')
ax.hist(np.array(recoMu_eta), bins=50,
        range=[-6, 6], histtype='step', normed=True, label='reco $\mu$')
...
ax.hist(np.array(res_phi[6]), bins=50, range=[-3.142,
                                              3.142], histtype='step', normed=True, label='t')
ax.hist(np.array(res_phi[24]), bins=50, range=[-3.142,
                                               3.142], histtype='step', normed=True, label='W')
...
ax.hist(np.array(res_phi[11]), bins=50, range=[-3.142,
                                               3.142], histtype='step', normed=True, label='e')
...
ax.hist(np.array(res_dR[13]), bins=50, range=[0, 8],
        histtype='step', normed=True, label='gen $\mu$')
