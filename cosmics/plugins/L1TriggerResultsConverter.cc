...
//
ff::L1TriggerResultsConverter::L1TriggerResultsConverter(const edm::ParameterSet& params)
...
ff::L1TriggerResultsConverter::~L1TriggerResultsConverter() {
...
void ff::L1TriggerResultsConverter::beginRun(edm::Run const&, edm::EventSetup const& setup) {
  mask_.clear();
  names_.clear();
  indices_.clear();
  if (legacyL1_) {
...
void ff::L1TriggerResultsConverter::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
//   auto out = std::make_unique<edm::TriggerResults>(l1bitsAsHLTStatus, names_);
  auto out = std::make_unique<edm::TriggerResults>(l1bitsAsHLTStatus, params_.id());
  iEvent.put(std::move(out));
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void ff::L1TriggerResultsConverter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(ff::L1TriggerResultsConverter);