                         ...
                         edm::ConsumesCollector&& cc)
{
  ...
  tree.Branch("muon_pfIso_neuHadEt_highThres", &muon_pfIso_neuHadEt_highThres_);
  tree.Branch("muon_pfIso_photonEt_highThres", &muon_pfIso_photonEt_highThres_);
  tree.Branch("muon_pfIso_puPt",               &muon_pfIso_puPt_              );

}

void
ffNtupleMuon::fill(const edm::Event& e,
                   const edm::EventSetup& es)
{
  ...
  for (const auto& muon : muons)
  {
    ...
    muon_gfit_p4_.push_back(
      ...
      LorentzVector(globalTrack->px(), globalTrack->py(), globalTrack->pz(), hypot(globalTrack->p(), M_Mu)) :
      LorentzVector(NAN,NAN,NAN,NAN)
    );
    muon_gfit_normChi2_   .emplace_back((globalTrack.isNonnull() and globalTrack->ndof()!=0) ? globalTrack->normalizedChi2() : NAN);
    muon_gfit_nValStaHits_.emplace_back(globalTrack.isNonnull() ? globalTrack->hitPattern().numberOfValidMuonHits() : -1);
    muon_gfit_tkQual_     .emplace_back(globalTrack.isNonnull() ? globalTrack->qualityMask() : -1);
    muon_gfit_algo_       .emplace_back(globalTrack.isNonnull() ? globalTrack->algo() : -1);
    muon_gfit_d0Sig_      .emplace_back(globalTrack.isNonnull() ? fabs(globalTrack->d0())/globalTrack->d0Error() : NAN);
    muon_gfit_dzSig_      .emplace_back(globalTrack.isNonnull() ? fabs(globalTrack->dz())/globalTrack->dzError() : NAN);
    muon_gfit_ptErr_      .emplace_back((globalTrack.isNonnull() and globalTrack->charge()!=0) ? globalTrack->ptError() : NAN);

    // innerTrack
    muon_ifit_good_       .emplace_back(innerTrack.isNonnull());
    muon_ifit_p4_.push_back(
      innerTrack.isNonnull() ?
      LorentzVector(innerTrack->px(), innerTrack->py(), innerTrack->pz(), hypot(innerTrack->p(), M_Mu)) :
      LorentzVector(NAN,NAN,NAN,NAN)
    );
    muon_ifit_normChi2_   .emplace_back((innerTrack.isNonnull() and innerTrack->ndof()!=0) ? innerTrack->normalizedChi2() : NAN);
    muon_ifit_nValHits_   .emplace_back(innerTrack.isNonnull() ? innerTrack->numberOfValidHits() : -1);
    muon_ifit_tkQual_     .emplace_back(innerTrack.isNonnull() ? innerTrack->qualityMask() : -1);
    muon_ifit_nLostHits_  .emplace_back(innerTrack.isNonnull() ? innerTrack->numberOfLostHits() : -1);
    muon_ifit_d0Sig_      .emplace_back(innerTrack.isNonnull() ? fabs(innerTrack->d0())/innerTrack->d0Error() : NAN);
    muon_ifit_dzSig_      .emplace_back(innerTrack.isNonnull() ? fabs(innerTrack->dz())/innerTrack->dzError() : NAN);
    muon_ifit_ptErr_      .emplace_back((innerTrack.isNonnull() and innerTrack->charge()!=0) ? innerTrack->ptError() : NAN);
    muon_ifit_algo_       .emplace_back(innerTrack.isNonnull() ? innerTrack->algo() : -1);
    muon_ifit_algoOrig_   .emplace_back(innerTrack.isNonnull() ? innerTrack->originalAlgo() : -1);
    muon_ifit_nTkLyrs_    .emplace_back(innerTrack.isNonnull() ? innerTrack->hitPattern().trackerLayersWithMeasurement() : -1);
    muon_ifit_nValPxlHits_.emplace_back(innerTrack.isNonnull() ? innerTrack->hitPattern().numberOfValidPixelHits() : -1);
    
    // outerTrack
    muon_ofit_good_       .emplace_back(outerTrack.isNonnull());
    muon_ofit_p4_.push_back(
      outerTrack.isNonnull() ?
      LorentzVector(outerTrack->px(), outerTrack->py(), outerTrack->pz(), hypot(outerTrack->p(), M_Mu)) :
      LorentzVector(NAN,NAN,NAN,NAN)
    );
    muon_ofit_normChi2_   .emplace_back((outerTrack.isNonnull() and outerTrack->ndof()!=0) ? outerTrack->normalizedChi2() : NAN);
    muon_ofit_nValStaHits_.emplace_back(outerTrack.isNonnull() ? outerTrack->hitPattern().numberOfValidMuonHits() : -1);
    muon_ofit_tkQual_     .emplace_back(outerTrack.isNonnull() ? outerTrack->qualityMask() : -1);
    muon_ofit_algo_       .emplace_back(outerTrack.isNonnull() ? outerTrack->algo() : -1);
    muon_ofit_ptErr_      .emplace_back((outerTrack.isNonnull() and outerTrack->charge()!=0) ? outerTrack->ptError() : NAN);
    muon_ofit_d0Sig_      .emplace_back(outerTrack.isNonnull() ? fabs(outerTrack->d0())/outerTrack->d0Error() : NAN);
    muon_ofit_dzSig_      .emplace_back(outerTrack.isNonnull() ? fabs(outerTrack->dz())/outerTrack->dzError() : NAN);


    // muon quality -- http://cmslxr.fnal.gov/source/DataFormats/MuonReco/interface/MuonQuality.h
    bool _isQualityValid = muon.isQualityValid();
    muon_qual_tkKink_    .emplace_back(_isQualityValid ? quality.trkKink : NAN); // value of the kink algorithm applied to the inner track stub
    muon_qual_chi2LocPos_.emplace_back(_isQualityValid ? quality.chi2LocalPosition : NAN); // chi2 value for the STA-TK matching of local position
    muon_qual_chi2LocMom_.emplace_back(_isQualityValid ? quality.chi2LocalMomentum : NAN); // chi2 value for the STA-TK matching of local momentum

    // muon id -- http://cmslxr.fnal.gov/source/DataFormats/MuonReco/interface/MuonSelectors.h
    muon_id_TMLastStationLoose_    .emplace_back(muon.isMatchesValid() ? muon::isGoodMuon(muon, muon::TMLastStationLoose) : NAN);
    muon_id_TMLastStationTight_    .emplace_back(muon.isMatchesValid() ? muon::isGoodMuon(muon, muon::TMLastStationTight) : NAN);
    muon_id_TM2DCompatibilityLoose_.emplace_back(muon.isMatchesValid() ? muon::isGoodMuon(muon, muon::TM2DCompatibilityLoose) : NAN);
    muon_id_TM2DCompatibilityTight_.emplace_back(muon.isMatchesValid() ? muon::isGoodMuon(muon, muon::TM2DCompatibilityTight) : NAN);
    muon_id_TMOneStationTight_     .emplace_back(muon.isMatchesValid() ? muon::isGoodMuon(muon, muon::TMOneStationTight) : NAN);
    ...
    muon_p4_               .push_back(LorentzVector(muon.px(), muon.py(), muon.pz(), muon.energy()));
    muon_pfP4_             .push_back(LorentzVector(muon.pfP4().px(), muon.pfP4().py(), muon.pfP4().pz(), muon.pfP4().energy()));
    muon_caloCompatibility_.emplace_back(muon.caloCompatibility()); /// Relative likelihood based on ECAL, HCAL, HO energy defined as L_muon/(L_muon+L_not_muon)
    ...
    muon_ecalTime_.emplace_back(muon.isEnergyValid() ? calEnergy.ecal_time : NAN);
    muon_hcalTime_.emplace_back(muon.isEnergyValid() ? calEnergy.hcal_time : NAN);
    ...
    muon_pfIso_chaHadPt_          .emplace_back(_isPFIsoValid ? pfIsoR03.sumChargedHadronPt : NAN);
    muon_pfIso_chaParPt_          .emplace_back(_isPFIsoValid ? pfIsoR03.sumChargedParticlePt : NAN);
    muon_pfIso_neuHadEt_          .emplace_back(_isPFIsoValid ? pfIsoR03.sumNeutralHadronEt : NAN);
    muon_pfIso_photonEt_          .emplace_back(_isPFIsoValid ? pfIsoR03.sumPhotonEt : NAN);
    muon_pfIso_neuHadEt_highThres_.emplace_back(_isPFIsoValid ? pfIsoR03.sumNeutralHadronEtHighThreshold : NAN);
    muon_pfIso_photonEt_highThres_.emplace_back(_isPFIsoValid ? pfIsoR03.sumPhotonEtHighThreshold : NAN);
