...
ffNtupleManager::ffNtupleManager(const edm::ParameterSet& ps)
{
  ...
  const std::vector<edm::ParameterSet>& ntuple_cfgs = ps.getParameterSetVector("Ntuples");
  for (const auto& ntuple_cfg : ntuple_cfgs)
  {
    const std::string& ntuple_name = ntuple_cfg.getParameter<std::string>("NtupleName");
    ff_ntuples_.emplace_back(ffNtupleFactory::get()->create(ntuple_name, ntuple_cfg));
...
void
ffNtupleManager::beginRun(const edm::Run& r,
...
void
ffNtupleManager::analyze(const edm::Event& e, 
                         const edm::EventSetup& es)
{
