...
ljGunProd::produce(edm::Event& e, const edm::EventSetup& es)
{
  ...
  for (const auto& dauPid : fPartIDs)
  {
    ...
    HepMC::GenParticle* parDau0 = new HepMC::GenParticle(
      HepMC::FourVector(dau0p4.Px(), dau0p4.Py(), dau0p4.Pz(), dau0p4.Energy()),
      dauPid,
      22 // status code. [21,29] would be assert as true for isHardProcess() by MCTruthHelper
    );
    ...
    HepMC::GenParticle* parDau1 = new HepMC::GenParticle(
      HepMC::FourVector(dau1p4.Px(), dau1p4.Py(), dau1p4.Pz(), dau1p4.Energy()),
      -dauPid,
      22
    );
  ...
  if ( fVerbosity > 0 )
  {
    std::cout << " LeptonJetGunProducer : End New Event Generation" << std::endl ;
    fEvt->print();
  }
}
