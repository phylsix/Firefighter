...
// fPDGTable( new DefaultConfig::ParticleDataTable("PDG Table") )
{
  ...
  if ( !rng.isAvailable() ) {
    throw cms::Exception( "Configuration" )
        << "The RandomNumberProducer module requires the "
  ...
  //
  // fPDGTablePath =
  // "/afs/cern.ch/sw/lcg/external/clhep/1.9.2.1/slc3_ia32_gcc323/data/HepPDT/" ;
  /*
    ...
    if ( !addPDGParticles( PDFile, tb ) ) { cout << " Error reading PDG !" <<
    endl; }
...
void
BaseFlatGunProducer::beginRun( const edm::Run& r, const EventSetup& es ) {
  es.getData( fPDGTable );
  return;
}
void
BaseFlatGunProducer::endRun( const Run& run, const EventSetup& es ) {}

void
BaseFlatGunProducer::endRunProduce( Run& run, const EventSetup& es ) {
